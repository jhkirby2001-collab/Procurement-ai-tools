...
3. **Anthropic API key rotation** flagged 2026-04-30, status unconfirmed. Production is rules-only, so not blocking — but verify before any AI re-run.
4. **Optional:** scrub remaining "Chicago FMPS" references from internal scripts/methodology if user requests. Currently intentional (factual data-source description, not branding).
