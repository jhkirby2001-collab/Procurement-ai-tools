...
2. **Confirm Streamlit Cloud subdomain rename** to `nigp-sourced-category-mapper.streamlit.app`. User said "i think" they did the UI rename — URL still not confirmed loading. **This cannot be checked from a Claude Code sandbox:** the environment's network policy denies CONNECT to `streamlit.app` (the proxy returns 403), so a curl there returns HTTP 000 — that is a blocked route, *not* evidence the site is down. Only James can settle it from a browser. Eight files cite a Streamlit URL (`README.md`, `build_sop_JHK3.py`, `build_case_study_JHK3.py`, `NIGP_Web_App_Factsheet_JHK3.md`, `streamlit_app.py`, this file) — fix them in one pass once the live URL is known.
3. **Raw/classified data tab in the Spend Report workbook.** Raised and deliberately deferred 2026-09-14 — design settled, nothing built. Ask: the download should carry the rows behind the numbers so a reader has visibility without hunting for the source extract. **Decision reached:** include the *classified* rows (the uploaded columns plus `Business_Category`, `NIGP_Class_3digit`, `NIGP_Item_5digit`, `Classification_Method`, `Classification_Confidence`, `Classification_Reason`), **not** a verbatim copy of the upload. The reasoning matters: a verbatim copy *is* redundant — the user already has that file — but the source extract cannot answer "why did this row land in Construction Materials", because the six classification columns exist only inside the tool. Pickable, ticked on by default (so a lean leadership deck can drop it). Rows = exactly what the analysis used. Safety cap ~100k with a visible truncation note. The data is already in the bundle as `_work`; `build_excel_report` does not currently receive it. Estimate ~45–60 min. **Risk to measure first:** `_style_data_sheet` touches every cell to set formats and borders — fine for a 40-row table, likely slow and memory-hungry at 25k rows. This tab probably needs a lighter writer (header styling only), so time the two paths before committing to the build.
4. **The row cap is a reconciliation trap — worth fixing regardless of item 3.** For an upload over 25,000 rows the app defaults to analyzing only the **first 25,000** (`max_rows` on the Spend Report page). Nothing in the workbook says so. Anyone reconciling the report against the source file therefore gets totals that do not match, with no explanation on the page or in the Excel. The report should state "analyzed N of M rows" somewhere unmissable — the Executive Summary sheet and the on-screen summary tiles.
5. **Anthropic API key rotation** flagged 2026-04-30, status unconfirmed. Production is rules-only, so not blocking — but verify before any AI re-run.
6. **Optional:** scrub remaining "Chicago FMPS" references from internal scripts/methodology if user requests. Currently intentional (factual data-source description, not branding).

**Done — do not re-raise:** `.docx` of `HOW_TO_USE_NIGP_Mapping_JHK3.md` (shipped as `outputs/HOW_TO_USE_NIGP_Mapping_JHK3.docx`, built by `build_how_to_operate_docx_JHK3.py`, committed in `e784316`). This sat in the open list long after it was finished and was re-raised across three sessions — verify a file exists before listing it as outstanding.
