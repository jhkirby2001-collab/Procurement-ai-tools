...
| [spend-analysis/scripts/spend_report_JHK3.py](spend-analysis/scripts/spend_report_JHK3.py) | Reusable **adaptive** spend-report engine behind the app's **Spend Report** page. Profiles every column (`profile_columns`), plans which analyses the file supports (`plan_analyses`), classifies (`classify_series`), and computes them all (`compute_all`): spend-by-category, Pareto 80/20, top vendors, consolidation, item-level same-item-across-vendors/departments (`item_consolidation`) and same-commodity-by-NIGP-code (`nigp_item_consolidation`), spend-by-department, spend-trend, tail-spend, vendor-concentration (HHI), single-vs-multi-source, category×department matrix, and spend-by-any-dimension (contract/diversity/status). Now **prescriptive**: a savings / cost-avoidance engine (addressable-spend methodology, benchmark rates) that splits identified savings into **hard (cashable) + cost avoidance**, annualizes and projects 3 years, and writes per-opportunity **recommended actions**. Executive summary leads with the savings headline; Excel adds **Savings Opportunity Summary**, **Top Consolidation Opportunities**, and **Sources & Methodology** tabs, with **professional matplotlib charts** embedded. **General & Other is absorbed via best-fit** so every row lands in one of the 17 categories. Rules-only classification, no AI. Grants fires only on strong grant signals on uploads. Runs standalone: `python spend-analysis/scripts/spend_report_JHK3.py <file>`. |
