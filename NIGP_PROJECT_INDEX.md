...
| File | What it is |
|---|---|
| [spend-analysis/data/reference/keyword_rules_DRAFT_JHK3.csv](spend-analysis/data/reference/keyword_rules_DRAFT_JHK3.csv) | 305 hand-curated keyword rules. The highest-volume hitters. |
...
| [spend-analysis/scripts/spend_report_JHK3.py](spend-analysis/scripts/spend_report_JHK3.py) | Reusable **adaptive** spend-report engine behind the app's **Spend Report** page. Profiles every column (`profile_columns`), plans which analyses the file supports (`plan_analyses`), classifies (`classify_series`), and computes them all (`compute_all`): spend-by-category, Pareto 80/20, top vendors, consolidation, spend-by-department, spend-trend, tail-spend, vendor-concentration (HHI), single-vs-multi-source, category×department matrix, and spend-by-any-dimension (contract/diversity/status). Data-driven executive summary + brand-colored multi-tab Excel with charts. Rules-only, no AI, format-agnostic. Every row lands in a real Business Category — unmatched rows go to the single catch-all "General & Other Procurement" (never an "Unclassified" gap), and the Spend-by-Category table carries an **Examples** column of representative descriptions per category. Grants fires only on strong grant signals on uploads. Runs standalone: `python spend-analysis/scripts/spend_report_JHK3.py <file>`. |
